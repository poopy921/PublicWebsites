...
var getNot: RegExp = /(?<!\\)(?:\\\\)*\~|(?<!\\)(?:\\\\)*NOT/gm;
var getThen: RegExp = /(?<!\\)(?:\\\\)*\-\>|(?<!\\)(?:\\\\)*THEN/gm;
var getIff: RegExp = /(?<!\\)(?:\\\\)*\<\-\>|(?<!\\)(?:\\\\)*IFF/gm;
var getEquals: RegExp = /(?<!\\)(?:\\\\)*\=\=|(?<!\\)(?:\\\\)*EQUALS/gm;
var getAll: RegExp =
    /(?<!\\)(?:\\\\)*qA|(?<!\\)(?:\\\\)*ALL|(?<!\\)(?:\\\\)*FOR EVERY/gm;
...
var descs = {
    ...
    "-equals":
        "The EQUALS statement says that a logical statement is equivalent to another logical statement.",
    ...
    "-symbolshort":
        "&& for AND<br>|| for OR<br>~ for NOT<br>-> for THEN<br><-> for IFF<br>== for EQUALS<br>qA for ALL (think of it as quantifierAll)<br>qE for EXISTS (think of it as quantifierExists)",
};
...
function wordsToLogicSa(): void {
    ...
    input = input.replace(/FOR EVERY|qA|ALL/gm, specialChars[6]);
    input = input.replace(/FOR SOME|qE|EXISTS/gm, specialChars[7]);
    ...
    input = input.replace(/==|EQUALS/gm, specialChars[5]);
    input = input.replace(/[ ]/gm, "");
    outputArea.value = input;
}
