...
var specialChars = "∧∨¬→↔≡∀∃≠";
var getOr: RegExp = /(?<!\\)(?:\\\\)*\|\||(?<!\\)(?:\\\\)*OR/gm;
var getAnd: RegExp = /(?<!\\)(?:\\\\)*\&\&|(?<!\\)(?:\\\\)*AND/gm;
var getNot: RegExp =
    /(?<!\\)(?:\\\\)*\!|(?<!\\)(?:\\\\)*\~|(?<!\\)(?:\\\\)*NOT/gm;
var getThen: RegExp = /(?<!\\)(?:\\\\)*\-\>|(?<!\\)(?:\\\\)*THEN/gm;
var getIff: RegExp = /(?<!\\)(?:\\\\)*\<\-\>|(?<!\\)(?:\\\\)*IFF/gm;
var getEquals: RegExp =
    /(?<!\\)(?:\\\\)*\=\=|(?<!\\)(?:\\\\)*EQUALS|(?<!\\)(?:\\\\)*EQUAL/gm;
var getAll: RegExp =
    /(?<!\\)(?:\\\\)*qA|(?<!\\)(?:\\\\)*ALL|(?<!\\)(?:\\\\)*FOR ALL|(?<!\\)(?:\\\\)*FOR EVERY/gm;
var getExists: RegExp =
    /(?<!\\)(?:\\\\)*qE|(?<!\\)(?:\\\\)*EXISTS|(?<!\\)(?:\\\\)*FOR SOME/gm;
var getNotEqual: RegExp =
    /(?<!\\)(?:\\\\)*!=|(?<!\\)(?:\\\\)*~=|(?<!\\)(?:\\\\)*NOT EQUALS|(?<!\\)(?:\\\\)*NOT EQUAL/gm;
var state = true;
...
var descs = {
    ...
    "-then":
        "The THEN, or conditional, statement only outputs false when the left side is true and the right side is false, and is true for any other combination",
    ...
    "-equals":
        "The EQUALS statement says that a logical statement is equivalent to another logical statement. There is also a NOT EQUALS statement that says the opposite: that the two logical statements are false.",
    ...
    "-exists":
        "The EXISTS quantifier says that at least one value in a domain x outputs True in a function f(x). It is only False when none of the values of x in f(x) equate to True.",
    "-symbolshort":
        "&& for AND<br>|| for OR<br>~ or ! for NOT<br>-> for THEN<br><-> for IFF<br>== for EQUALS (you can also add a NOT for not equals)<br>qA for ALL (think of it as quantifierAll)<br>qE for EXISTS (think of it as quantifierExists)",
};
...
function wordsToLogic(): void {
    var input: string = inputArea.value;
